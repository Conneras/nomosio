...
const PAGES = [
  ...
  { slug: 'symmetoxi',  sections: ['symmetoxi', 'stirixi'],  nav: 'Συμμετοχή',
    ...
    desc: 'Δήλωσε ΠΑΡΩΝ και δες τα επόμενα βήματα του Νομόσιου.' },
...
function homeHub() {
  const cards = [
    ...
    { href: '/symmetoxi', t: 'Συμμετοχή', d: 'Δήλωσε ΠΑΡΩΝ και δες τα επόμενα βήματα. Χωρίς εσένα δεν κουνιέται τίποτα.', i: '✊' }
  ];
...
function fixLinks(html, onPage) {
  return html.replace(/href="#([a-z]+)"/g, (full, id) => {
    ...
    if (!page) return full;
    // αλλιώς → σύνδεσμος σελίδας· η άγκυρα μένει όταν η ενότητα δεν είναι η πρώτη της σελίδας
    const target = PAGES.find(x => '/' + x.slug === page);
    return `href="${page}${target && target.sections[0] !== id ? '#' + id : ''}"`;
  });
}
